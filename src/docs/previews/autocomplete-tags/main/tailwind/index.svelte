<script lang="ts">
	...
	const options: { [key: string]: Choice[] } = {
		...
		savory: [{ label: 'Basil' }, { label: 'Bacon' }, { label: 'Rosemary' }],
	};
...
<div class="flex flex-col gap-1">
	...
	>
		{#each $selected as t}
			<div
				...
				class="flex items-center overflow-hidden rounded-md bg-magnum-200 text-magnum-900 [word-break:break-word]
						data-[disabled]:bg-magnum-700
			data-[highlighted]:bg-magnum-300 data-[disabled]:hover:cursor-default
		...
		/>
		<ChevronDown class="float-right my-auto square-5" />
	</div>
