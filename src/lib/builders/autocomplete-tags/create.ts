...
import type {
	...
	AutocompleteTagsItemProps,
	CreateAutocompleteTagsProps,
} from './types.js';
 ...
 */
export function createAutocompleteTags<
	Value,
	Selected extends Array<AutocompleteTagsOption<Value>> = Array<AutocompleteTagsOption<Value>>
>(props?: CreateAutocompleteTagsProps<Value, Selected>) {
	const withDefaults = { ...defaults, ...props } satisfies CreateAutocompleteTagsProps<
		Value,
		Selected
	>;
	...
	function getTags(el: HTMLElement) {
		return Array.from(el.querySelectorAll(selector('tag'))).filter((el): el is HTMLElement =>
			isHTMLElement(el)
		);
	}
	...
	const isSelected = derived([selected], ([$value]) => {
		return (item: Value): boolean =>
			$value?.map((o) => o.value).some((o) => deepEqual(o, item)) ?? false;
	});
				...
				addMeltEventListener(node, 'keydown', async (e) => {
					...
					if (e.key === kbd.ARROW_LEFT || e.key === kbd.ARROW_RIGHT) {
						...
						if (rootEl) {
							...
							if ($currentItem !== null && candidateNodes.includes($currentItem)) {
								...
								e.preventDefault();
							} else if (e.key === kbd.ARROW_RIGHT && node.selectionStart === node.value.length) {
					...
					if (e.key === kbd.BACKSPACE && node.value === '') {
						...
						if (rootEl) {
							...
							if (tags.length > 0) selectItem(last(tags));
						}
						return;
					}
		...
		returned: ([$selected]) => {
			return (tag: AutocompleteTagsItemProps<Value>) => {
				...
				const selected = disabled
					? undefined
					: $selected?.map((o) => o.value).some((o) => deepEqual(o, tag.value)) ?? false;
	...
	const deleteTrigger = builder(name('delete-trigger'), {
		...
		returned: ([$selected]) => {
			return (tag: AutocompleteTagsItemProps<Value>) => {
				...
				const selected = disabled
					? undefined
					: $selected?.map((o) => o.value).includes(tag.value) ?? false;
