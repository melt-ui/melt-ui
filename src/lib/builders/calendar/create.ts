...
>(props?: CreateCalendarProps<Multiple, Value, S>) {
	 ...
	 */
	const headingValue = derived(
		[months, locale, monthsHeaderType],
		([$months, $locale, $monthsHeaderType]) => {
	...
	const fullCalendarLabel = derived(
		...
		([$headingValue, $calendarLabel]) => {
			const strHeadingValue =
				$headingValue instanceof Array ? $headingValue.join(' - ') : $headingValue;

			return `${$calendarLabel}, ${strHeadingValue}`;
		}
	);
